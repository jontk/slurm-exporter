...
func TestPartitionCollectorUtilities(t *testing.T) {
	...
	t.Run("ParsePartitionState", func(t *testing.T) {
		testCases := []struct {
			input    string
			expected string
		}{
		...
		for _, tc := range testCases {
			result := collector.parsePartitionState(tc.input)
			if result != tc.expected {
				t.Errorf("parsePartitionState(%s) = %s, expected %s", tc.input, result, tc.expected)
			}
		}
	})

	t.Run("ParseTimeLimit", func(t *testing.T) {
		testCases := []struct {
			input       string
			expectedMin int64
			shouldError bool
		...
		for _, tc := range testCases {
			result, err := collector.parseTimeLimit(tc.input)
			if tc.shouldError && err == nil {
				t.Errorf("parseTimeLimit(%s) should have returned error", tc.input)
			}
			if !tc.shouldError && err != nil {
				t.Errorf("parseTimeLimit(%s) returned unexpected error: %v", tc.input, err)
			}
			if !tc.shouldError && tc.expectedMin > 0 {
				expectedDuration := time.Duration(tc.expectedMin) * time.Minute
				if result != expectedDuration {
					t.Errorf("parseTimeLimit(%s) = %v, expected %v", tc.input, result, expectedDuration)
				}
			}
		}
	})
